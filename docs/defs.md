...
# control-loop
Is a fundametal building block of [control-systems](#control-system) (which manages, commands, direct or regulates the bahavior or other devies or systems) in industrial control systems.
...
- closed-control-loops: in this the output of managed-system is "fed back" to the [control-system](#control-system) and influences the control-action
...
# closed-control-loop
Here, we adopt the defintion of 'control-loop' from Kubernetes. https://kubernetes.io/docs/concepts/architecture/controller/.  Kubernetes concept of sending [current-state](#current-state) as input to the [controller](#controller) is de facto a feedback mechanism, hence every control-loop in Kubernetes is closed-control-loop.
...
The ultimate goal of control-system is to solve or keep in harmony the [management-problem](#management-problem).

# control-feedback
A representation of [current-state](#current-state) sent by the [managed-system](#managed-system) to the [control-system](#control-system).

# control-action
An action send to or performed on [managed-system](#managed-system) by the [control-system](#control-system) that has to bring it closer to [desired-state](#desired-state).
...
# ingress-agent
When [user](#user) wants to integrate his [managed-system](#managed-system) with Lupus - as in any other case of combining two softwares together -  he needs some API Integration. Sometimes [user](#user) cannot modify the [managed-system](#managed-system). For such case, and for more generic secure option we came up with ingress-agent. Ingress-Agent links Lupus and [managed-system] together. Typical operation for Ingress-Agent operations is to:
...
- translate [managed-system](#managed-system) nomenclature to the Lupus one

Communication of Ingress-Agent and [managed-system](#managed-system) is case-specific, out of scope of this repo and to be done by [user](#user). But communication of Ingres-Agent and Lupus is standardized as [lupin-interface](#lupin-interface). 
...
# loop logic 
Same as [reconciliation-logic](#reconcilliation-logic), while reconciliation logic describes more what has to be done in a single iteration and loop-logic is more of a general loop attribute.

In context of this repo, we can highlight a [computing-part of the loop-logic](#computing-part). Computing part is responsible for the actual calculations, while the other part of loop-logic contain sequence of these calculations or data fed from/to them.

# computing-part
Part of the [loop-logic](#loop-logic) that is responsible for calculations that have to be done for reconciliation. 

Since Lupus is [data-driven](#data-driven), this part is performed by [external-elements](#external-element), not [lupus-elements](#lupus-element).
...
# user-functions
It was assumed that [controller](#controller) of [lupus-element](#lupus-element) should not contain any [computing-part](#computing-part) of [loop-logic](#loop-logic). But sometimes it is non-sense to deploy a HTTP Server solely for the reason of it serving as [external-element](#external-element) for Lupus (e.g. in case if [computation-part](#computation-part) of [loop-logic](#loop-logic) contains few lines of code). We allow to deploy such logic in [controller](#controller) of [lupus-element](#lupus-element) by introduction of "user-functions". These functions act logically as [external-elemenet](#external-element) with the difference that physically they are deployed in Kubernetes cluster, not outside of it. 
...
# management-problem
A problem that occurs in given [managed-system](#managed-system) and has to be solved by [control-system](#control-system). 
...
Adjusting a [managed-system](#managed-system) with [Ingress](#ingress-agent) or [Egress Agents](#egress-agent) or development of [external-elements](#external-element) will always require software developers and such people are included in "user" definition, but we distinguish the role of a [designer](#designer). 

# designer

Our assumption was that the design of [loop-workflow](#loop-workflow) will require minimal set of technical/engineering skills. Thus, the person who solely operates on [LupN](#lupn) and delegates work related to [Ingress](#ingress-agent) and [Egress Agents] or [external-elements](#external-element) to Software Engineers is denoted by the name "loop-designer". Its mission is to design a loop. [User](#user), in contrary, denotes whole organisation.

# lupus-controllers

An abbreviation for [master](#master) and [lupus-element](#lupus-element) [controllers](#controller).
...
# final-data
In each iteration when the Data will be modified with all the elements's actions we name such Data  - "final". It is simply the Data at the output of element.
...
# custom-resources
This is a Kubernetes term: https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/
...
# crd
This is a Kubernetes term: https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/
